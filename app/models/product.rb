class Product < ApplicationRecord
  ...
  def self.import(file)
    ...
    CSV.foreach(file.path, options) do |row|
      ...
      p    "=========================================================================== =========================================================================== ========================================================================== #{(row.to_hash["Категория"].split(", ").last ? (Category.all.map {|cat| cat.name }.include?(row.to_hash["Категория"].split(", ").last) ? Category.where(name: row.to_hash["Категория"].split(", ").last) : 1) : 1)}"
      #     #other_desc:
      #     #general_category:
      #     #max_age:
